...
- `ollama-proxy`: relays only Ollama traffic from `claude-code` to the host Ollama endpoint.

This split enforces isolation because Claude is not attached to any network with direct internet egress.
